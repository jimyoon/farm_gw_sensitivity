...
# S = 0.2        # Storativity = Specific yield 
# m = 200        # Aquifer thickness/depth [m]
# K = 0.1         # K value [m/d]
# WL = 30       # Initial depth to water [m]
...
def Analytical(S, m, K, WL, R, IrrDepth, years):
    ...
    days = [30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80] # days of irrigation per year 
    ...
    for i in range(aquifer_df.iloc[:,0].size):
        if aquifer_df.m[0] - aquifer_df.WL[0] > 100:
            ...
            if aquifer_df.s[i]/100 > 0.6 * aquifer_df.m[i] or aquifer_df.Trans[i] == 0 or aquifer_df.s[i] > 50 or aquifer_df.Area[i] == 0:
                aquifer_df.viable[i] = 0
    ...
    # Starting Transmissivity not to exceed 100 m of saturated thickness 
    T_array = np.zeros((aquifer_df.iloc[:,0].size, years+1)) # tracks Transmissivity through time
    ...
    DTW_ts = np.zeros((aquifer_df.iloc[:,0].size, 730*years)) # depth to water at half-day resolution 
    WL_array = np.zeros((aquifer_df.iloc[:,0].size, years+1)) # starting water level at beggining of each annual period 
    ...
    max_drawdown = aquifer_df.s[row]
    sat_thickness =  (D_array[row,0]-aquifer_df.WL[row])
    
    for t in range(100): # years of pumping
        if T_array[row,t] > 0 and entrance_V < 0.5 and max_drawdown/sat_thickness < 0.8 : # conditionals to determine if pumping is possible 
            Q_gpm = aquifer_df.Q[row]
            Q = Q_gpm*1440*1/264*365 # pumping rate in GPM converted, GPD (mult by 1440), to m^3/day (divide by 264), to m^3/year
            Qv = Q_gpm*1440*1/264*aquifer_df.Days[row] # pumped volume (m^3) per yr 
            r = 0.5 # radial distance of drawdown estimate
    ...
    Ct = np.zeros((1, years+30))
    Cc = np.zeros((1, years+30))
    Cm = np.zeros((1, years+30))
    ...
    cost_per_one_eight_cell = np.zeros((1, years+30))
    ...
    if DTW_ts[row,0] < 0 and abs(DTW_ts[row,0]) < aquifer_df.m[row] and aquifer_df.viable[row] > 0: 
        ...
        maintenance = 0.07
        if aquifer_df.Q[row] < 200:
            well_unit_cost = 50
        else:
            well_unit_cost = 75 
        
        if aquifer_df.m[0] - aquifer_df.WL[0] < 100:
            Ini_cost = well_unit_cost*3.28*aquifer_df.m[row] # $75 per foot, converted to $/meter
            CapCost = Ini_cost*(1+rate)**n*(rate/((1+rate)**n-1)) # cap cost per year
            Cc[0,:] = np.tile(CapCost,years+30) 
            Cm[0,:] = maintenance*Ini_cost # maintenance cost [% of initial cost]
            
        else:
            Ini_cost = well_unit_cost*3.28*D_array[row,0] # $75 per foot, converted to $/meter

            for i in range(len(Ct[0,:])-30):
                if i == 0:
                    CapCost = Ini_cost*(1+rate)**n*(rate/((1+rate)**n-1))
                    Cc[0,i] = CapCost  
                    Cm[0,i] = maintenance*Ini_cost # maintenance cost [% of initial cost]
                    
                elif (i+1)/n - (i+1)//n == 0: 
                    Ini_cost = well_unit_cost*3.28*D_array[row,i]
                    CapCost = Ini_cost*(1+rate)**n*(rate/((1+rate)**n-1)) # new cap cost per year for 30 yr period
                    Cc[0,i] = Cc[0,i]+CapCost
                    Cm[0,i] = maintenance*Ini_cost
                    
                elif D_array[row,i+1] - D_array[row, i] > 0: # add additional capital cost if depth is increased
                    Cc[0,i] = Cc[0,i-1]
                    Cc[0,i+1:i+31] = well_unit_cost*3.28*(D_array[row,i+1] - D_array[row,i])*(1+rate)**n*(rate/((1+rate)**n-1)) 
                    Ini_cost = well_unit_cost*3.28*D_array[row,i+1]
                    Cm[0,i] = maintenance*Ini_cost
        ...
        for i in range(years):
            ...
            if Annual_avg_PWL_depth[0,i] > aquifer_df.m[row] or Annual_avg_PWL_depth[0,i] == 0:
                ...
                Ct[0,i] = Elec_cost[0,i]+Cc[0,i]+Cm[0,i]
    ...
    unit_cost_per_acre_ft = Ct/(aquifer_df.Qv[row]/1233.48)
...
# axs[0,0].plot(unit_cost_per_acre_ft[0,0:years])
# axs[0,0].set_ylabel('$/acre-ft')
# axs[0,1].plot(Cc[0,0:years]+Cm[0,0:years])
# axs[0,1].set_ylabel('Capital + O&M')
# axs[1,0].plot(Ct[0,0:years])
# axs[1,0].set_ylabel('Total')
# axs[1,1].plot(Elec_cost[0,0:years])
# axs[1,1].set_ylabel('Energy')
# plt.tight_layout()

# plt.figure()
# plt.plot(total_pumped_volume[0,0:years], unit_cost[0,0:years])
# plt.ylabel('$ per m^3')
# plt.xlabel('Pumped Volume (km^3)')
# #plt.ylim([0.03, 0.14])
